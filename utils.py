...
def _load_llm_config():
    with open('llm-config.yaml', 'r') as f:
        yaml_data = yaml.safe_load(f)  # Changed from yaml.load to yaml.safe_load
    return yaml_data
...
def fetch_model_config():
    ...
    print(f"Loaded config type: {type(llm_config)}")
    print(f"Loaded config: {llm_config}")
    print(f"Looking for model_name: {chosen_model_name}")
    
    # Check if llm_config is a dict and has 'models' key
    if isinstance(llm_config, dict) and "models" in llm_config:
        for model_config in llm_config["models"]:
            if chosen_model_name == model_config.get("model_name"):
                print(f"Found matching model: {model_config.get('model')}")
                return model_config.get("model")
        # If no match found, return default
        default = llm_config.get("default_model")
        print(f"No match found, returning default: {default}")
        return default
    else:
        # Fallback if YAML structure is unexpected
        print("WARNING: Unexpected YAML structure, using fallback")
